...
from data import ImageLabeledSet, PixelLabeledSet, DeviceDataloader
from net import FreezeModel, PFeedback
...
# 初始化配置
cfg = Config()
parseConfig("./conf/config.ini", cfg)
...
# 初始化网络模型
segModel1 = create_model(cfg.model.name, in_channels=3, out_channels=cfg.data.num_classes)
segModel1 = FreezeModel(segModel1)
segModel1.to(cfg.train.device)

segModel2 = create_model(cfg.model.name, in_channels=3, out_channels=cfg.data.num_classes)
segModel2 = FreezeModel(segModel2)
segModel2.to(cfg.train.device)

segModel = create_model(cfg.model.name, in_channels=3, out_channels=cfg.data.num_classes)
